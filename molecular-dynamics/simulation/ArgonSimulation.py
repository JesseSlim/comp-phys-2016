...
class ArgonSimulation:
	...
	def update_coordinates(self):
		...
		if self.keepRVFData:
			...
			self.r += self.h * self.v_hat % self.L
		...
		if tSinceEq >= 0:
			if tSinceEq == 0:
				if self.keepRVFData:
					self.rEquilibriumStart = np.copy(self.rNoPBC[t,:,:])
					self.vsqEquilibriumStart = np.mean(np.sum(self.v[t,:,:]**2, axis=1))
				else:
					self.rEquilibriumStart = np.copy(self.rNoPBC)
					self.vsqEquilibriumStart = np.mean(np.sum(self.v**2, axis=1))
			if self.keepRVFData:
				deltaXYZ = self.rNoPBC[t,:,:] - self.rEquilibriumStart
			else:
				deltaXYZ = self.rNoPBC - self.rEquilibriumStart
				
			self.deltaRsq[tSinceEq] = np.mean(np.sum(deltaXYZ**2, axis=1))
