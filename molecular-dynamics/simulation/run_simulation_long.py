...
sim = ArgonSimulation()
sim.rescalingPeriod = 40
sim.rescalingTime = 40.0
sim.equilibrationTime = 50.0
sim.simulationTime = 110.0
...
for M in Ms:
    for T in Ts:
        for rho in rhos:
            ...
            print("T:   " + str(T))
            print("rho: " + str(rho))
            filename = "results-longrun/argon-M-%d-T-%.2f-rho-%.2f-simjob-%d.npz" % (M,T,rho,simJob)
